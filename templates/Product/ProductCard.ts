  ...
  setup(props) {    
    ...
    const ProdcutCard = defineAsyncComponent({
      loader: async () => {
        try {
          ...
          return await import(`/src/components/template_${props.template}/Product/ProductCard.vue`);
        } catch (error) {
          // If the specified template fails to load, fall back to SimpleProduct1.vue
          return import(`/src/components/template_01/Product/ProductCard.vue`);
        }
      },
    });
    ...
    const promotionTag = defineAsyncComponent({
      loader: async () => {
        try {
          // Attempt to dynamically import the specified template component
          return await import(`/src/components/Others/promotionTag.vue`);
        } catch (error) {
          // If the specified template fails to load, fall back to SimpleProduct1.vue
          return import(`/src/components/Others/promotionTag.vue`);
        }
      },
    });
