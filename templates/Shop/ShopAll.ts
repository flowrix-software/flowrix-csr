  ...
  setup(props) {
    ...
    const ShopTemplate = defineAsyncComponent({
      loader: async () => {
        try {
          ...
          return await import(`/src/components/template_01/Shop/ShopAll.vue`);
        } catch (error) {
          // If the specified template fails to load, fall back to SimpleProduct1.vue
          return import(`/src/components/template_01/Shop/ShopAll.vue`);
        }
      }
    });
    const PaginationComponent = defineAsyncComponent({
      loader: async () => {
        try {
          // Attempt to dynamically import the specified template component
          return await import(`/src/components/Others/Pagination.vue`);
        } catch (error) {
          // If the specified template fails to load, fall back to SimpleProduct1.vue
          return import(`/src/components/Others/Pagination.vue`);
        }
      },
    });
     ...
     const ProductCard = defineAsyncComponent(() =>
      import(`/src/components/Product/ProductCard.vue`)
    )
